...
void MilkReceptionAddFrame::insertReceptionMilk()
{
    ...
    if (isNoDeliverers || isNoMilkPoints) {
        if (isNoDeliverers) {
            if (ViewUtils::yesNoWarnMsgBox(this, tr("Чтобы внести сдачу молока необходим сдатчик. "
                                                      "Желаете добавить сейчас?"))) {
                m_mainWindow->addDeliverer();
            }
        }
        if (isNoMilkPoints) {
            if (ViewUtils::yesNoWarnMsgBox(this, tr("Чтобы внести сдачу молока необходим молокопункт. "
                                                      "Желаете добавить сейчас?"))) {
    ...
    const auto idDeliverer = ViewUtils::getCurValueFromItemViewModel(ui->listViewDeliverers,
                                 DT_ID).toLongLong();
    ...
    const auto idMilkPoint = ViewUtils::getCurValueFromComboBoxModel(ui->comboBoxMilkPoints,
                                          MPT_ID).toLongLong();
